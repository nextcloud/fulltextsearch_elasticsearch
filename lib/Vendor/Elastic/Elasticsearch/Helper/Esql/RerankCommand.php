...
class RerankCommand extends EsqlBase
{
    ...
    protected function renderInternal() : string
    {
        if (\sizeof($this->fields) == 0) {
            throw new RuntimeException("The rerank command requires one or more fields to rerank on");
        }
        if ($this->inference_id == "") {
            throw new RuntimeException("The rerank command requires an inference ID");
        }
        ...
        return "RERANK " . $query . " ON " . \implode(", ", \array_map(array($this, "formatId"), $this->fields)) . " WITH " . \json_encode($with);
    }
}
