...
class CompletionCommand extends EsqlBase
{
    ...
    protected function renderInternal(): string
    {
        ...
        if ($this->named_prompt) {
            return "COMPLETION " . $this->formatId(array_keys($this->named_prompt)[0]) . " = " . $this->formatId(array_values($this->named_prompt)[0]) . " WITH " . json_encode($with);
        } else {
            return "COMPLETION " . $this->formatId($this->prompt) . " WITH " . json_encode($with);
        }
    }
}
