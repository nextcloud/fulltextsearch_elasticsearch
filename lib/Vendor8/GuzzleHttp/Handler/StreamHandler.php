...
namespace OCA\FullTextSearch_Elasticsearch\Vendor8\GuzzleHttp\Handler;

use OCA\FullTextSearch_Elasticsearch\Vendor8\GuzzleHttp\Exception\ConnectException;
use OCA\FullTextSearch_Elasticsearch\Vendor8\GuzzleHttp\Exception\RequestException;
use OCA\FullTextSearch_Elasticsearch\Vendor8\GuzzleHttp\Exception\TransferException;
use OCA\FullTextSearch_Elasticsearch\Vendor8\GuzzleHttp\Multiplexing;
use OCA\FullTextSearch_Elasticsearch\Vendor8\GuzzleHttp\Promise as P;
use OCA\FullTextSearch_Elasticsearch\Vendor8\GuzzleHttp\Promise\FulfilledPromise;
use OCA\FullTextSearch_Elasticsearch\Vendor8\GuzzleHttp\Promise\PromiseInterface;
use OCA\FullTextSearch_Elasticsearch\Vendor8\GuzzleHttp\Psr7;
use OCA\FullTextSearch_Elasticsearch\Vendor8\GuzzleHttp\TransferStats;
use OCA\FullTextSearch_Elasticsearch\Vendor8\GuzzleHttp\TransportSharing;
...
class StreamHandler
{
    ...
    public function __construct(array $options = [])
    {
        foreach ($options as $name => $_) {
            if (!isset(self::KNOWN_CONSTRUCTOR_OPTIONS[$name])) {
                \OCA\FullTextSearch_Elasticsearch\Vendor8\trigger_deprecation('guzzlehttp/guzzle', '7.14', \sprintf('The "%s" StreamHandler constructor option is unknown; guzzlehttp/guzzle 8.0 will reject unknown constructor options.', (string) $name));
            }
        }
    ...
    private static function triggerUnsupportedRequestOptionDeprecations(RequestInterface $request, array $options): void
    {
        if (\array_key_exists('curl', $options) && $options['curl'] !== null && $options['curl'] !== [] && !self::isCurlOptionGeneratedByAuth($options)) {
            \OCA\FullTextSearch_Elasticsearch\Vendor8\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "curl" request option to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because the stream handler ignores cURL options.');
        }
        if (\array_key_exists('expect', $options) && $options['expect'] !== \false && $request->hasHeader('Expect')) {
            \OCA\FullTextSearch_Elasticsearch\Vendor8\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "expect" request option to the stream handler is deprecated when it adds an Expect header; guzzlehttp/guzzle 8.0 will reject this option because the stream handler does not support Expect: 100-Continue.');
        }
    }
    private static function triggerConflictingStreamContextOptionDeprecations(array $streamContext): void
    {
        ...
        foreach ($streamContext as $wrapper => $contextOptions) {
            ...
            foreach ($contextOptions as $option => $_) {
                ...
                \OCA\FullTextSearch_Elasticsearch\Vendor8\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing stream_context.%s.%s in the "stream_context" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $wrapper, $option, $conflictingOptions[$wrapper][$option]));
            }
        }
    }
    private static function triggerUnsupportedStreamContextOptionDeprecations(array $streamContext): void
    {
        ...
        \OCA\FullTextSearch_Elasticsearch\Vendor8\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing PHP stream context options outside the built-in stream handler allow-list to the "stream_context" request option is deprecated; guzzlehttp/guzzle 8.0 will reject stream context options outside the allow-list. Deprecated option%s: %s.', \count($unsupportedOptions) === 1 ? '' : 's', \implode(', ', $unsupportedOptions)));
    }
    /**
     * @return string[]
     */
