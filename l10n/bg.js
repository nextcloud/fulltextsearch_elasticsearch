OC.L10N.register(
    "fulltextsearch_opensearch",
    {
    "Full text search - OpenSearch Platform" : "OpenSearch Platform - платформа за търсене на пълен текст",
    "Index using OpenSearch" : "Индексиране с помощта на OpenSearch",
    "Extension to the _Full text search_ app to communicate with OpenSearch." : "Разширение към приложението _Full text search_ за комуникация с OpenSearch.",
    "Open Search" : "Open Search /Еластично търсене/",
    "Address of the Servlet" : "Адрес на сървлета",
    "Include your credential in case authentication is required." : "Включете вашите идентификационни данни, в случай че е нужно удостоверяване.",
    "Index" : "Индекс",
    "Name of your index." : "Име на вашия индекс.",
    "[Advanced] Analyzer tokenizer" : "[Разширен] Анализатор токенизатор",
    "Some language might needs a specific tokenizer." : "Някой език, може да се нуждае от специфичен токенизатор. ",
    "_" : "_"
},
"nplurals=2; plural=(n != 1);");
