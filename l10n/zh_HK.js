OC.L10N.register(
    "fulltextsearch_opensearch",
    {
    "Full text search - OpenSearch Platform" : "全文搜尋 - OpenSearch 平台",
    "Index using OpenSearch" : "使用 OpenSearch 索引",
    "Extension to the _Full text search_ app to communicate with OpenSearch." : "_Full text search_ 應用程式的擴充元件，用來與 OpenSearch 通訊。",
    "Open Search" : "Open Search",
    "Address of the Servlet" : "Servlet 的位置",
    "Include your credential in case authentication is required." : "如果需要身分驗證，請包含您的憑證。",
    "Index" : "索引",
    "Name of your index." : "您索引的名稱。",
    "[Advanced] Analyzer tokenizer" : "[進階] 分析器 token 生成器",
    "Some language might needs a specific tokenizer." : "某些語言可能需要特定的 tokenizer。",
    "object" : "物件"
},
"nplurals=1; plural=0;");
